...
class PyMail():
    ...
    is from.
    eg:  python pymail.py -s /home/xyz/PyBulkMail/test.html /home/xyz/PyBulkMail/pymail.csv 'Email Subject'
    """
    def __init__(self, html_path, csv_path, subject, *args, **kwargs):
        self.html_path = html_path
        self.csv_path = csv_path
        self.subject = subject
        self.from_name = kwargs.get('from_name', config.FROM_NAME) #for multiple email_address and names
        self.from_email = kwargs.get('to_name', config.FROM_EMAIL) #for multiple email_address and names
    ...
    def _html_parser(self, recipient_data):
        ...
        if recipient_data:
            for key, value in recipient_data.items():
                placeholder = "<!--%s-->" % key
                html_content = html_content.replace(placeholder, value) #to insert name of person in mail
    ...
    def send(self, retry_count=0, recipient_list=None):
        ...
        for recipient_data in recipient_list:
            ...
            smtp_server = smtplib.SMTP('smtp.mail.yahoo.com', 587, timeout=10)
            smtp_server.set_debuglevel(1)
            try:
                smtp_server.starttls() #for secured sending of mails
                smtp_server.login('xyz@yahoo.in', 'xyz123')
...
def main(sys_args):
    ...
    try:
        ...
        print "Not enough arguments supplied. eg: python pymail.py -s /home/xyz/PyBulkMail/test.html /home/xyz/PyBulkMail/pymail.csv 'Email Subject'"
        sys.exit()
    ...
    pymail = PyMail(html_path, csv_path, subject)

    if action == '-s':
        if raw_input("You are about to send to %s recipients. Do you want to continue (yes/no)? " % pymail.count_recipients()) == 'yes':
            # save the csv file used to the stats file
            pymail._stats("CSV USED: %s" % csv_path)
        ...
        print "%s option is not support. Use [-s] to send to all recipients" % action

    # save the end time to the stats file
    pymail._stats("END TIME: %s" % datetime.now())
